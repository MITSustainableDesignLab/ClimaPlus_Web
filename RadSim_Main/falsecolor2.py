...
def showHelp():
    ...
    ("-loc", "LOCATION", "this is the LOCATION for the radiation map"), # to save specific data for a location, like the max scale val
...
class FalsecolorOptionParser(FalsecolorBase):
    ...
    def __init__(self, logger=None, args=[]):
	    ...
	    '-loc'    : ('setLocation',     self._validatePath,   False),
...
class FalsecolorLegend(FalsecolorBase):
    ...
    def createColorScale(self):
        ...
        if self.is_vertical():
            # fix yres for now
            args = "-e v=(y+.5)/(yres+.0000000000001);vleft=v;vright=v;vbelow=(y-.5)/(yres+.0000000000001);vabove=(y+1.5)/(yres+.0000000000001);"
    ...
    def createText(self):
        ...
        ## print text list to a file
        with open ('colorValues.txt', 'w+') as outfile:
            outfile.write("%s\n" % textlist)
...
class FalsecolorImage(FalsecolorBase):
    ...
    def findAutoScale(self):
        ...
        ## printing to file
        #with open(LOCATION+'/autoscale.txt', 'w+') as outfile:
            #for item in extreme.split():
                #outfile.write(item + ' , ')
        ...
        for d in [0.001,0.01,0.1,1,10,100,1000,10000,100000,1000000]:
		    ...
		    #with open(self.location+'/autoscale.txt', 'w+') as outfile:
		    with open('autoscale.txt', 'w+') as outfile:
    ...
    # defining a function to identify the location of the rad simulation
    ...
    def showExtremes(self):
        ...
        cmd = "pextrem -o"
        with open('extremes.txt', 'w') as outfile:
            outfile.write('self input:\n')
            outfile.write(self._input+'\n')
        extreme = self._popenPipeCmd(cmd, self._input+"\n")
        ...
        #with open('extremes_n.txt', 'w') as outfile:
            #outfile.write('max value: '+str(maxval)+'\n')
            #outfile.write('min value: '+str(minval)+'\n')
